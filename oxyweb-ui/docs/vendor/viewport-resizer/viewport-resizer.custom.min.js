...
/* Embed edition */
(function (o, y, O) {
	var ca = "https:" === location.protocol ? "http://" : "http://", da = y.createElement("script"), ea = y.getElementsByTagName("script")[0];
	da.src = ca + "ajax.googleapis.com/ajax/libs/jquery/2.0.1/jquery.min.js";
	ea.parentNode.insertBefore(da, ea);
	y.close();
	var fa = function (a) {
		if (o.jQuery && jQuery.fn.jquery === "2.0.1")a(jQuery); else {
	...
	fa(function (a) {
		...
		function A(b, e) {
			var d = b ? b : a("a.active", n).add(a("a.active", H)), h = b ? d.attr("data-viewport").toDimension() : {width: B, height: I}, i = e == "swap" ? {width: h.height, height: h.width} : {width: h.width, height: h.height}, g = i.width + "x" + i.height, f = RegExp(e == "swap" ? h.width + "x" + h.height : h.height + "x" + h.width, "g");
			...
			d.attr("title", l);
			ja.attr("class", d.is('[data-viewport="auto"]') ? "auto" : "").css({width: i.width, height: i.height, "margin-left": -i.width / 2});
			if (!b) {
		...
		function Da() {
			...
			a("#bookmarklet", C).attr({href: "javascript:void((function(d){if(self!=top||d.getElementById('toolbar')&&d.getElementById('toolbar').getAttribute('data-resizer'))return false;d.write('<!DOCTYPE HTML><html style=\"opacity:0;\"><head><meta charset=\"utf-8\"></head><body>" + b + '<script src="' + Ea + "\"><\/script></body></html>')})(document));"})
		}
		...
		function Fa() {
			function b(k) {
				...
				j = k ? "" : "scale(" + j + ")";
				k = k ? "" : "0 0";
				v.css({zoom: j, "-webkit-transform": j, "-webkit-transform-origin": k, "-moz-transform": j, "-moz-transform-origin": k, "-ms-transform": j, "-ms-transform-origin": k, "-o-transform": j, "-o-transform-origin": k, transform: j, "transform-origin": k})
			}

			function e(k) {
				v.css({"min-width": k ? "" : u + "px", "min-height": k || !l ? "" : l + "px"})
			}
			...
			var d = function () {
				...
				return k
			}(), h = d.width || false, i = d.height || false;
			d = d["initial-scale"] || false;
			var g = true, f = true, l = false;
			if (Q) {
				b("clear");
				e("clear");
				return false
			}
			if (h)if (h !== "device-width")u = Number(h); else f = g = false; else u = 980;
		...
		function la() {
			...
			if (!ma) {
				...
				a.event.special.mousewheel = {setup: function () {
					if (this.addEventListener)for (var f = b.length; f;)this.addEventListener(b[--f], d, false); else this.onmousewheel = d
				}, teardown: function () {
					if (this.removeEventListener)for (var f = b.length; f;)this.removeEventListener(b[--f],
						d, false); else this.onmousewheel = null
				}};
				a.fn.extend({mousewheel: function (f) {
					return f ? this.bind("mousewheel", f) : this.trigger("mousewheel")
				}, unmousewheel: function (f) {
					return this.unbind("mousewheel", f)
				}});
				var d = function (f) {
					var l = f || o.event, k = [].slice.call(arguments, 1), j = 0, q = 0, s = 0;
					f = a.event.fix(l);
					f.type = "mousewheel";
			...
			T && S.css({overflow: "scroll", "-webkit-overflow-scrolling": "touch", width: "100%", height: "100%"});
		...
		function Ia(b) {
			function e(h) {
				a("[data-viewport].active", "#devices").attr({"data-viewport": h.height + "x" + h.width}).orientationClassName();
				return{width: h.height, height: h.width}
			}

			var d = a("a.active", n).attr("data-viewport").toDimension();
			if (b === 0 || b === 180) {
				if (d.width > d.height)d = e(d)
			} else if (b === 90 || b === -90)if (d.height > d.width)d =
				e(d);
			ga(d)
		}
		...
		function Ha() {
			var b = {Version: "Viewport Resizer (" + Ja + ")", "User agent": K, "Meta viewport": m.find("meta[name=viewport]:first", "head").attr("content")};
			X.html("");
		...
		function ra() {
			...
			D.css("color", function (e) {
				...
				var d = parseInt(e[1]), h = parseInt(e[2]);
				e = parseInt(e[3]);
		...
		var Ja = "1.1.4", L = ca + "lab.maltewassermann.com/viewport-resizer/", Y = L + "resizer.min.css", Ea = L + "resizer.min.js", t = a(o), sa = a("html"), U = a("head"), F = a("body"), /* <embed> */G = a("[data-viewport-source]").data('viewport-source') || a(location).attr("href")/* </embed> */, V = "n/a", K = typeof navigator.userAgent ===
			"undefined" ? V : navigator.userAgent, ma = /webkit/i.test(K), T = /mobile.*safari/i.test(K);
		L = /opera/i.test(K);
		var Ka = a('<meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1">');
		Y = a('<link rel="stylesheet" href="' + Y + '">');
		var ta = a('<style type="text/css" media="print">'), C = a('<div id="expand">').hide(), X = a("<dl>"), La = a('<a id="bookmarklet">').attr("data-text", "Your bookmarklet").append(a("<span>").html("↔ Resizer")), pa = a('<span class="invisible">').text(" has been changed. Please save or update your bookmarklet."),
			...
			D = a('<div id="container">'), w = a('<div id="wrapper">'), p = a('<iframe id="content" name="content" frameborder="0">').attr("src", G), wa = a('<b id="handle-e">'), xa = a('<b id="handle-s">'), aa = a('<b id="handle-w">');
		G = a('<b id="handle-se">');
		var ya = a('<b id="handle-sw">'), J = wa.add(xa).add(aa).add(G).add(ya), ja = a('<div id="phantom">');
		G = new Date;
		...
		String.prototype.toDimension = function () {
			if (this ==
				"auto")return{width: ba, height: za - 42, auto: true}; else {
				var b = this.match(/\d{1,}\d/g);
				if (b) {
					b = b.slice(0, 2);
					return{width: parseInt(typeof b[0] === "undefined" ? 400 : b[0]), height: parseInt(typeof b[1] === "undefined" ? 600 : b[1]), auto: false}
				} else return null
			}
		};
		...
		(function () {
			...
			function d() {
				return{w: parseInt(w.css("width")) || w.width() || 0, h: parseInt(w.css("height")) || w.height() || 0}
			}
			...
			J.on({mouseenter: function () {
				...
				g.preventDefault();
				i = a(this);
				h = {w: d().w, h: d().h, x: g.pageX, y: g.pageY};
				t.on({mousemove: b, mouseup: e});
		...
		r.on("edit", function () {
			...
			function () {
				...
				if (b) {
					...
					b = a("#add").text("").attr({"data-viewport": b.width + "x" + b.height}).setProperties();
					z(b);
					a(this).trigger("blur")
		...
		(function () {
			function b() {
				...
				if (g > f - x)d = false; else if (g === e)d = true;
				if (d)g += x % f; else g -= x % f
			}

			var e = 200, d = false, h = false, i, g;
			Z.on("start", function () {
				i = true;
				g = Math.round(B / x) * x;
				b();
		...
		p.on("load", function () {
			...
			/* <embed> */
			if(window.location != window.parent.location && parent.$) {
				// Notify observers from parent window:
				window.parent.$(document).trigger('viewport-resizer.ready');
				// IE8+ fix:
				window.parent.postMessage('viewport-resizer.ready', '*');
			}
			/* </embed> */
		...
		var Ba = false, Ca = false, Na = [17, 224, 91, 93];
